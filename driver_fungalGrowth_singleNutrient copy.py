...
try:
    ...
    params, config = hf.get_configs('D:\Fungi_new\parameters.ini')
except:
...
def driver_singleNutrient(run):
    ...
    mycelia,mycelia_2 = sf.mycelia_dict()
    ...
    if (whichInitialCondition == 1):
        ...
        mycelia_2, num_branches_2, num_total_segs_2, dtt_2 = sf.initial_conditions_line_2(mycelia_2, num_segs, x_vals, y_vals)
        ...
        mycelia_2, num_branches_2, num_total_segs_2, dtt_2 = sf.initial_conditions_cross_2(mycelia_2, num_segs, x_vals, y_vals)
    
    print('current dtt is',dtt)
    print('current dtt_2 is',dtt_2)
    ...
    while current_time < params['final_time']: 
        ...
        while(this_time < ntimes):
            ...
            mycelia_2 = nf.uptake(sub_e_gluc, mycelia_2, num_total_segs_2, var_nutrient_backgrnd, params['dt_e'],params)
            # mycelia = nf.uptake(sub_e_gluc, sub_e_treha, mycelia, num_total_segs, var_nutrient_backgrnd)
            tU_1 = time.time()
            time_uptake += (tU_1 - tU_0)
            #if dtt == dtt_2: print('dtt = dtt_2')
        # end while
        
        # RELEASE
        mycelia = nf.release(sub_e_treha, mycelia, num_total_segs, isTipRelease, params)
        mycelia_2 = nf.release(sub_e_treha, mycelia_2, num_total_segs_2, isTipRelease, params)
        if (np.isnan(np.sum(mycelia['cw_i'][:num_total_segs]))):
            breakpoint()
        if (np.isnan(np.sum(mycelia_2['cw_i'][:num_total_segs_2]))):
            breakpoint()
        ...
        mycelia_2 = nf.transloc(mycelia_2, params, num_total_segs_2, dtt_2, isActiveTrans,
                              whichInitialCondition,
                              isConvectDependOnMetabo_cw,
                              isConvectDependOnMetabo_gluc,
                              isConvectDependOnMetabo_treha)
        ...
        old_num_total_segs_2 = num_total_segs_2
        # breakpoint()
        ...
        mycelia_2, num_total_segs_2, dtt_2 = gf.extension(mycelia_2, params, num_total_segs_2, 
                                        dtt_2, x_vals, y_vals,isCalibration, 
                                        dist2Tip_new, fungal_fusion,
                                        chance_to_fuse, branch_rate)
        if(np.any(np.isnan(mycelia['cw_i'][:num_total_segs]))):
            breakpoint()
        if(np.any(np.isnan(mycelia_2['cw_i'][:num_total_segs_2]))):
            breakpoint()
        ...
        if any(np.where(mycelia_2['can_branch'])[0]):
            reached_max_branches, mycelia_2, num_total_segs_2, dtt_2 = gf.branching(mycelia_2, params, 
                                        num_total_segs_2, dtt_2, x_vals, y_vals, 
                                        isCalibration, dist2Tip_new, 
                                        fungal_fusion, restrictBranching,
                                        chance_to_fuse, adj_branch_rate)
        if reached_max_branches == True:
            print ('Reached maximum number of branches:', np.max(mycelia['cw_i'])[0])
            print ('Reached maximum number of branches:', np.max(mycelia_2['cw_i'])[0])
            breakpoint()
            break
        if (old_num_total_segs > num_total_segs):
           breakpoint()
        ...
        # if current_step % (1*160) == 0:
            # breakpoint()
            hf.plot_fungus(mycelia, mycelia_2, num_total_segs, current_time, folder_string, param_string, params, run)
            ...
            num_branches_2 = max(mycelia_2['branch_id'])[0]+1
    ...
    t_1 = time.time()
    num_branches = max(mycelia['branch_id'])[0]+1
    num_branche_2 = max(mycelia_2['branch_id'])[0]+1
    num_segs = num_total_segs
    num_tips = np.count_nonzero(mycelia['is_tip'])
    max_radii = max(np.sqrt(mycelia['xy2'][:,0]**2 + mycelia['xy2'][:,1]**2))
    avg_radii = np.mean(np.sqrt(mycelia['xy2'][:,0]**2 + mycelia['xy2'][:,1]**2))
